...
class TinyPngUploader:
    def __init__(self, input_dir, thumb_input_dir, resize_input_dir, width=None, height=None):
        self.threads = list()
    ...
    def run(self, thumb_list, convert_list, resize_list):
        ...
        for file in resize_list:
            thread = threading.Thread(target=lambda: self.resize(os.path.join(self.resize_input_dir, file), self.width, self.height))
            self.threads.append(thread)
            thread.start()
...
if __name__ == '__main__':
    ...
    png = TinyPngUploader(input_dir, input_thumbs_dir, INPUT_RESIZE_DIR, width=268, height=183)
    png.run(input_files_for_thumbs, input_files, input_files_for_resize)
